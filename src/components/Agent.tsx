...
function getRoleStyles(role: InspectionAgent['role']) {
  switch (role) {
    case 'engineer':
      return { head: 'bg-yellow-400', body: 'bg-yellow-600', limb: 'bg-yellow-700', eye: 'bg-black', aura: 'bg-yellow-300/20', pose: 'engineer-pose' }
    case 'courier':
      return { head: 'bg-cyan-400', body: 'bg-cyan-600', limb: 'bg-cyan-700', eye: 'bg-white', aura: 'bg-cyan-300/20', pose: 'courier-pose' }
    case 'security':
      return { head: 'bg-red-500', body: 'bg-red-700', limb: 'bg-red-900', eye: 'bg-yellow-300', aura: 'bg-red-400/20', pose: 'security-pose' }
    case 'admin':
      return { head: 'bg-purple-400', body: 'bg-purple-600', limb: 'bg-purple-800', eye: 'bg-cyan-300', aura: 'bg-purple-300/20', pose: 'admin-pose' }
    default:
      return { head: 'bg-gray-400', body: 'bg-gray-600', limb: 'bg-gray-700', eye: 'bg-black', aura: 'bg-gray-300/20', pose: '' }
  }
}
  ...
  return (
    ...
    >
      {(agent.status === 'working' || agent.status === 'idle') && (
        <div className={`absolute min-w-[148px] rounded border px-2 py-1 shadow-[0_0_10px_rgba(6,182,212,0.3)] backdrop-blur-md z-50 ${agent.status === 'working' ? '-top-11 border-cyan-500/50 bg-cyan-950/90 text-cyan-300' : '-top-9 border-cyan-500/30 bg-cyan-950/75 text-cyan-200/90'}`}>
          <div className="whitespace-nowrap font-tech text-[10px] tracking-wider">{agent.task}</div>
